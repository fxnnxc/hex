...
def construct_lrp(model, device):
    ...
    model.to(device)
    layers, rules = construct_cnn_layers_and_rules(model)
    
    lrp_model = LRP(layers, rules, device=device, mean=mean, std=std)
    return lrp_model
  
def construct_cnn_layers_and_rules(model):
    layers = [] 
    rules = [] 
    # Rule is z_plus
    for layer in model.c1: # Convolution 
        layers.append(layer)
        rules.append({"z_plus":True, "epsilon":1e-6})

    for layer in model.c2: # Convolution 
        layers.append(layer)
        rules.append({"z_plus":True, "epsilon":1e-6})
        
    for layer in model.c3: # Convolution 
        layers.append(layer)
        rules.append({"z_plus":True, "epsilon":1e-6})

        
    for layer in model.final: # Convolution 
        layers.append(layer)
        rules.append({"z_plus":True, "epsilon":1e-6})
